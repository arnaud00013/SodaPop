...
if (as.integer(num_fitness_landscape) != 7){
  ...
  tryCatch(expr ={
    ggplot(data=subset(df_results_variants_analysis_current_sim,Generation_ctr==(sort(unique(Generation_ctr),decreasing = TRUE)[sample(1:3,1)]))) + geom_density(mapping = aes(x = nb_uniq_species_of_gene)) + scale_x_continous(breaks=1:nb_species_simulated,limits=c(1,nb_sepecies_simulated)) + xlab("Gene mobility") + ylab("Density")
  ...
  #average_cai vs ps at the end of simulation
  df_subset_near_end_sim_current_reg <- subset(df_results_variants_analysis_current_sim,Generation_ctr%in%(sort(unique(Generation_ctr),decreasing = TRUE)[1])) #if you want to use random time sampling near end of simulation, use [sample(1:3,1)] instead of [1]
  v_pos_for_current_reg <-intersect(which(is.finite(df_subset_near_end_sim_current_reg$avg_cai)),which(is.finite(df_subset_near_end_sim_current_reg$ps)))
  tryCatch(expr = ggplotRegression(fit=lmp(avg_cai ~ ps, data = df_subset_near_end_sim_current_reg[v_pos_for_current_reg,],na.action=na.omit,center=FALSE),ggsave_path=save_plots_workspace_current_sim,the_filename=paste0("avg_cai_vs_ps_sim_",simulation_name,"_at_END_OF_SIMULATION_FOR_ALL_MBGS.png"),xlabl = "ps",ylabl = "average CAI"),error=function(e) print(paste0("Regression skipped because of following error : ",e)))
