...
std::map<std::string, double> get_codon_usage_map(const int the_cell_id, const std::string& the_cell_file_workspace){
    ...
    if (b_f_does_exit){
        ...
        std::string msg_err = "Error! The following .cell file does not exist : "+std::string(buffer);
...
std::vector<int> get_unique_species(){
    ...
    for (auto it = ref_subpops_abundance_map.begin(); it != ref_subpops_abundance_map.end();++it){
...
std::map<int,int> get_ref_map_subpops_abundance(){
    bool b_f_does_exit = false;
    std::string line;
    std::string path_file = "files/start/population.dat";
    ...
    if (b_f_does_exit){
        ...
        while (!the_pop_ifs.eof()) {
            ...
            if (word == "C"){
                ...
                std::string query_str(".cell");
                std::size_t i = path_cell_file.find(query_str);
                if (i != std::string::npos){//erase .cell from string
                    path_cell_file.erase(i, 5); //erase .cell from string
                }
                query_str = "files/start/";
                i = path_cell_file.find(query_str);
